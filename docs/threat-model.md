...
- **What the freeze does, measured where marked** — Ethereum mainnet fork at block 25,815,805 against the real USDC proxy, impersonating the live blacklister (`test/Erc20HtlcSpike.t.sol` in the counter-leg contract repo `pyrxd-eth-htlc`; summarised in `docs/solutions/design-decisions/usdc-corridor-is-issuer-trusted-not-trustless.md`):
...
- **What the gate deliberately does NOT check:** the **refundee**, at claim time. A `claim` sweeps to the claimant, so a frozen refundee cannot make it revert; refusing there would be a guard refusing valid work, and would hand the counterparty a free unilateral veto over a leg the claimant has already earned. The refundee's freeze matters *before funding* — and **no pre-fund freeze gate exists today**: `assert_not_frozen_before_reveal` has exactly one production caller, `Erc20HtlcLeg.claim`. A party who funds a leg whose refundee is already frozen finds out at refund time.
