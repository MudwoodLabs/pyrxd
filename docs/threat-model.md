...
**Reach:** Significant for privacy, limited for theft. The default mainnet config ships two public ElectrumX servers run by different operators (`network/registry.py`: `electrumx.radiant4people.com` and `electrumx.radiantcore.org`). What the second one buys is narrow. It is **failover, not a quorum**: `FailoverElectrumXClient` moves a read to the next server only when the first fails it — a transport fault, an error answer or a malformed response, never a node's policy verdict — and that server's client is dropped and the server that does answer is promoted ahead of it. The one exception is an indexer read (`wave.*`, `glyph.*`, `swap.*`, which `RxinDexerClient` sends idempotent): a server that answers that it does not implement the method is passed over but kept, and not demoted for that answer. So an ordinary read (balance, UTXOs, history, confirmations, headers) is still whatever the one answering server says, and a hostile primary is still a single point of trust for it. Indexer reads are narrower still: of the two, only `electrumx.radiantcore.org` runs the RXinDexer extension (`electrumx.radiant4people.com` answers with JSON-RPC `-32601`; measured 2026-09-24 for `wave.resolve`, `wave.reverse_lookup`, `glyph.get_token`, `glyph.get_recent` and `swap.get_orders`), so with the defaults, WAVE name and Glyph token lookups are answered by that one server. The one place pyrxd uses the pair as two *independent* sources is HashMark §7.6 form 2 (`glyph inspect --wave-name`, `verify --wave-name`). There, the name binding and the mark's block height must come from different servers; a mutable glyph's chain candidates and the proof that its tip is unspent must come from different servers; and every block height the verdict compares (the mark's and each chain step's) must be reported identically by both servers, or the verdict degrades to form 1 with the disagreement as its reason. Each server's height for the mark is bound to the block its node names: the header at that height must hash to the transaction's `blockhash` (searched two blocks either side). The chain steps' heights are compared between the servers but are not checked against headers. The `tip − confirmations + 1` arithmetic alone was one block low on every server at once for a few seconds after each block, which two agreeing servers cannot catch. "Two servers" means two different HOSTS. Endpoints on one host are one source, whatever else differs: case, a trailing dot, the port, the path or the query, or the same IP address spelled another way (IPv6 zero-compression, an IPv4-mapped IPv6 address, or the hex, octal, decimal and short dotted IPv4 forms `inet_aton` accepts). A hostname and its IP address, or two DNS names for one machine, cannot be detected from URLs and count as two, so configuring one server under two such names defeats the rule. What that does NOT cover: the name→glyph binding is still the indexer server's claim. pyrxd checks that the glyph's own mint payload names the label asked about, but not that this glyph is the registration in force (a duplicate or lapsed claim names it too). And no height is verified: the header binding checks a server against itself, and nothing checks proof-of-work or merkle inclusion, so two servers that tell the same lie still move the answer. So the pair bounds what one hostile server can move alone, and does not help if both servers lie together. testnet and regtest ship no server at all.
...
### TA10: Token issuer / bridge governance

**Capabilities:** The entity that controls the ERC-20 contract holding a counter leg — Circle for native USDC, Tether for USDT, the L2 bridge for an OP-stack bridged token. It can **freeze (blacklist) an address**, which makes every `transfer` touching that address revert; on a bridged token it can **burn an arbitrary holder's balance**; on an upgradeable token its admin can replace the token logic outright. It cannot forge signatures, cannot touch the Radiant leg, and is not a party to the swap — it does not need to know the swap exists.

**Goals:** Usually none directed at the swap. This actor acts for its own reasons — a sanctions listing, a law-enforcement order, an incident response — and a counterparty behind an affected address is collateral. It is listed anyway because **intent is irrelevant to the outcome**: the effect on a locked HTLC is identical whether or not the freeze was aimed at it. Freeze is a designed-in property of the asset, not a hypothetical — roughly **$110M frozen across ~500 USDC addresses**, and the 2025 GENIUS Act **mandates** the capability for US issuers (external research fetched 2026-08-22, recorded in `docs/solutions/design-decisions/usdc-corridor-is-issuer-trusted-not-trustless.md`; not a measurement by this project).

**Reach:** Unique in this document: every other actor sits outside the protocol, this one sits inside the **asset**. Freezing the HTLC **contract** address strands the tokens permanently — `claim` and `refund` both revert, with no timeout that rescues them (S25, measured). Nothing in pyrxd prevents that and nothing recovers from it; the controls only decide whether the frozen party has already published the preimage when it lands. The actor exists **only on the ERC-20 counter leg** — the native ETH, BTC and RXD legs have no issuer — and `Erc20Token.has_blacklist` in `src/pyrxd/eth_wallet/tokens.py` records, per pinned token, whether it applies at all.
...
- **Control(s) — what BOUNDS it (it is not eliminated):** the cross-clock timelock margin sizes `t_rxd` to open no earlier than the counter-leg deadline PLUS the finality-stall-tolerant margin (`eth_absolute_to_rxd_relative_blocks`; until 2026-09-23 this control read "strictly before ... minus", the pre-#482 relation, which the sizer has not implemented since #482); the reorg-finality gate refuses an unsafe early claim (SAFE/WAIT/SQUEEZED, never a silent claim); the **value-scaled claim burial** (red-team 2026-06-12 HIGH, now enforced) requires the taker's claim to bury deep enough that reorging it costs ≥ the value at stake; and the proactive-refund window `N` is coupled to the finality+burial reserve so a reveal cannot be timed into a squeeze the taker could otherwise have acted in.
- **What bounds the automation (correct the record):** a `ClaimExecutor` now exists (`watch/claim_executor.py`), but it is **not** an auto-claim hot wallet. It is **keyless for the asset** — it scrapes the maker's already-public preimage and broadcasts the *keyless* covenant claim whose `output[0]` is pinned to the taker holder PKH, so it **cannot redirect the asset** (the watchtower holds no value key; a stolen *fee* key can only burn dust fees). It is **dormant-by-construction** (declines unless an operator wires a resolver + writes a per-swap covenant sidecar), and as of 0.9.0 it is **armed-by-exception**: on a value-bearing network it broadcasts nothing unless `enable_autonomous_mainnet_custody=True`. Autonomous claim size is bounded by `claim_dust_ceiling` (a default the operator raises with explicit per-value consent; the blunt `accept_unbounded_reorg_risk` flag cannot cross it). So R1's closure for an *un-armed or un-wired* tower still rests on **operator/taker liveness within `t_rxd`**; an *armed* tower closes it autonomously within the consented value bound. The earlier "no `ClaimExecutor` in v1" statement is superseded by this paragraph.
- **Residual risk:** REAL and ACCEPTED (same as the BTC↔RXD direction) — this is the inherent HTLC free option, not a pyrxd bug. Surfaced loudly (never a silent `COMPLETED`). The autonomous closer above shrinks the operator-liveness window *for armed towers within the consented value bound*; outside that (un-armed, or value above the configured ceiling) size `t_rxd` for the worst-case pin/eviction window the taker must survive online. The external audit + a genuine two-party adversarial run remain the gates before relying on autonomous custody for non-dust value.
...
- **Why it is unrepairable — Radiant supports NEITHER RBF NOR CPFP** (verified against `Radiant-Core` @ `afdf57b1` and the live mainnet node, Radiant Core 3.1.2, 2026-08-09):
  - **No RBF.** `src/validation.cpp:667` and `:866` reject any mempool conflict outright (`txn-mempool-conflict`, `REJECT_DUPLICATE`). The only `bip125` string in the tree is a **deprecated** help label; there is no `bumpfee` RPC. Radiant ships **DSProof** — it treats a conflict as *fraud to broadcast*, the exact opposite of replacement.
  - **No CPFP.** `getmempoolancestors` / `getmempooldescendants` exist and make it *look* supported, but `src/miner.cpp:404` selects on `GetModifiedFeeRate()` = `(nFee + feeDelta) / vsize` — the transaction's **own** fee over its **own** size — and `break`s at `blockMinFeeRate`. The `backlog` queue is topological ordering only. **A high-fee child cannot lift a low-fee parent into a block.**
  - **The window.** `DEFAULT_MEMPOOL_EXPIRY = 8` hours (`src/validation.h:82`). The stuck transaction squats on its own inputs for up to 8 hours before they free for a rebuild. **If the deadline falls inside that window there is no remedy** — the claim never confirms, `t_rxd` elapses, and the counterparty refunds.
  - Note the *inverse* of the Bitcoin intuition: BIP125 mempool **pinning** (the two-party run plan's NA-2b) does **not** apply to Radiant, because nothing is replaceable. The risk that replaces it is worse and was previously unnamed.
- **Control:** fee **pre-sizing** — the only control that exists, and therefore mandatory rather than nice-to-have.
  - `pyrxd.gravity.fee_policy.DeadlineFeePolicy` derives the requirement as `ceil(size × rate / 1000)` from the node's advertised `effective_minrelaytxfee` (0.10 RXD/kB on the reference node; `minrelaytxfee` is 0.01 — the **effective** rate is what binds), with a deadline-scaled premium inside a 6-block horizon. The rate is **injected**, never hardcoded as a constant: it is node policy and it moves.
  - `htlc_spend.build_htlc_claim_tx` / `build_htlc_refund_tx` refuse to return an under-fee'd transaction, sized against `len(tx.serialize())` — the exact wire bytes, after signing, never an estimate.
  - `radiant_leg.RadiantCovenantLeg` applies the **deadline-aware** requirement immediately before broadcast: for a claim, blocks-to-deadline is `t_rxd − covenant confirmations` (the maker's CSV refund branch opens at that depth), so a claim racing a closing window is fee'd for it. A shortfall **refuses and PAGEs** (`InsufficientFundsError`, logged at ERROR) rather than emitting an unfixable transaction; `watch/claim_executor.py` maps it to `DECLINED` (which pages) and deliberately retries on the next tick, because `CappedFeeWalletSource` is dispense-once and the next dispense may be a larger input.
  - That retry is only safe because the cap charges **spend, not dispense**. The fee input has to be dispensed *before* the transaction can be built (its value and script are inputs to the build), so a refused build had already committed the input and charged the cumulative cap for a transaction that never reached a node. Measured: a pool of 7 × 500,000 + 1 × 20,000,000 photons under a 20,000,000 cap burned 3,500,000 of the cap on seven refusals that broadcast nothing, and the eighth dispense then raised `FeePoolExhaustedError` — *"dispensing 20000000 photons would exceed total_cap_photons=20000000 (already dispensed 3500000)"* — with the covering input funded, unspent, and unreachable, while the asset ran out its deadline to the counterparty's CSV refund. `radiant_leg._unspent_on_failure` now reports the input back via `CappedFeeWalletSource.release_unspent` on any pre-broadcast failure, which credits the cap without rewinding the dispense-once cursor (so no input can back two transactions, and a small head-of-line input cannot re-refuse forever).
  - The historical flat 546-photon dust floor is retained as a floor but is **not** the requirement: at the reference rate a ~266-byte RXD claim needs ~2,660,000 photons, about **4,900× the dust floor**.
...
- **Why the asymmetry is structural** (verified against the shipped state machine, not inferred):
  ...
  - The taker's own refund is the **shorter** timelock — `assert_timelock_margin` enforces `t_rxd - t_btc >= margin` since #482 (the maker holds `p` and LOCKS the Radiant leg, so that leg carries the longer timeout). **This bullet said the opposite until 2026-09-01**, calling `t_btc - t_rxd >= margin` "deliberate and correct"; that is the layout in which the maker refunds its own leg and still claims the counter leg with `p`. The consequence stated here inverts with it: the party immobilised longest is the **maker**, on the leg it locked, not the taker. The griefing residual itself stands — capital lockup at near-zero attacker cost — but its cost asymmetry needs re-deriving.
...
- **Control:** operational, not protocol. Order the legs by trust (taker-locks-first is a default, not a consensus rule — have an untrusted counterparty lock first); size `t_btc` deliberately, since below the ceiling the margin sets (#482: the counter leg must refund a full margin before the maker's Radiant refund, in wall clock) it is chosen, and a longer one directly lengthens the griefing window; keep counterparty state at the negotiation layer; and **do not auto-accept orders from unknown counterparties** — automation is the risk multiplier, not the attack.
...
- **Action:** The mirror of S23, and the cheaper attack. The maker publishes the envelope — which fixes `H`, the timelocks and the BTC claim key it holds — and then locks **nothing**. The taker funds its BTC (or ETH) HTLC. The maker immediately claims it with the `p` it has held since it generated the secret. There is no asset to claim back, and the taker's own refund does not open until `t_btc`.
- **Asset:** A8, real funds. A **one-sided taker loss of the full `btc_sats`** — not a shortfall, the whole leg.
- **Why consensus permits it:** the BTC claim leaf is `<H> OP_SHA256 OP_EQUALVERIFY <makerClaimPk> OP_CHECKSIG`. It carries **no precondition that the asset was ever locked**. The FSM's `NEGOTIATED --> BTC_LOCKED : taker funds FIRST` ordering is *bookkeeping*, not a safety guarantee, and `maker_claims_btc` refuses the premature claim only for an **honest** maker driving its own coordinator — a hostile maker does not use a coordinator. Documented as hazard **HZ-1** in `docs/htlc-handshake-wire-format.md`, whose normative rule is: *a taker MUST NOT fund the counter leg until it has confirmed the maker's asset lock on chain, at the agreed scriptPubKey, for the agreed value, at a depth the taker chose.*
- **Why it went unenforced:** the check existed only inside `scripts/btc_swap_two_host.py` / `scripts/eth_swap_two_host.py`. Any caller driving `SwapCoordinator` directly got `pre_btc_lock_check(...) -> ok=True` and `taker_funds_btc(...) -> BTC_LOCKED` having invoked **zero** methods on the Radiant leg — the library never read the Radiant chain at all.
- **Controls:**
  - `SwapCoordinator.taker_verify_asset_funding` — the taker's independent, fail-closed gate, run as **step 5 of `pre_btc_lock_check`** and therefore on every `taker_funds_btc`.
  - `RadiantCovenantLeg.verify_maker_asset_funded` — re-derives the covenant scriptPubKey from the **taker's own `terms`** (never one the maker advertises), locates its funded UTXO, binds the **on-chain value to `terms.radiant_amount` exactly**, and requires a **confirmation depth**. "Funded" alone is not enough: ElectrumX `listunspent` includes mempool outputs, so a maker can fund with a replaceable transaction, wait for the lock, then double-spend the funding away.
  - **Re-run at lock time** inside `taker_funds_btc`, immediately before the counter-leg broadcast, closing the verify→lock TOCTOU. It sits before the `SeenStore` reserve so the reserve keeps its "last step before the only broadcast" property and a refusal does not burn `H`.
  - Depth pin from existing policy: a real-value (`MarginPolicy.is_measured`) swap requires `rxd_claim_burial` confirmations on the covenant funding — the same depth the claim-finality gate requires of the taker's own Radiant claim. An estimated/test policy defers to the leg's `min_confirmations` (the operator's `--taker-min-rxd-confs`).
  - Fail-closed everywhere: an unfunded SPK, a mis-valued or ambiguous covenant UTXO, a shallow funding, an unreachable node, and a `radiant_leg` that does not implement the read all refuse to fund.
  - `tests/test_taker_asset_funding_gate_adversarial.py` (never-funded, mis-valued, wrong script, 0-conf, measured-policy depth, unreachable node, leg without the capability, verified-then-vanished TOCTOU, honest control).
- **Residual risk:** same chain-source caveat as S23 — the gate is only as good as the Radiant node behind it, and it reads a **single** ElectrumX endpoint. There is no RXD-side quorum reader equivalent to `MultiSourceBtcFundingReader`; for real value the taker should read a node it controls. Not exercised in a live two-party run; only in tests.

### S25: Issuer freeze strands an ERC-20 counter leg — the atomicity residual (TA10)

- **Action:** The counter leg is USDC or USDT held in a per-swap `Erc20Htlc` contract. The issuer blacklists an address involved in that leg while the swap is locked. If it is the **contract's** address the tokens are simply gone — `claim` reverts, `refund` reverts, and there is no third path. If it is the **claimant's**, the HTLC ordering does the damage: the claimant publishes the preimage in order to take the other leg, so a freeze landing in that window lets the counterparty sweep their side while the frozen party can claim nothing.
- **Asset:** A-cross (the ERC-20 counter leg). A **one-sided loss** of the whole leg; for a contract freeze, a permanent loss to *nobody* — the tokens end up neither party's.
- **What the freeze does, measured where marked** — Ethereum mainnet fork at block 25,815,805 against the real USDC proxy, impersonating the live blacklister (`test/Erc20HtlcSpike.t.sol`, which lives in the separate counter-leg contracts repository and is **not reproducible from this repo**; summarised in `docs/solutions/design-decisions/usdc-corridor-is-issuer-trusted-not-trustless.md`):
  - **claimant frozen** — `claim` reverts (`test_frozen_claimant_cannot_claim`). That `refund` still works is **inferred, not run**: no test exercises it.
  - **refundee frozen** — `refund` reverts, `claim` works. **Inferred, not measured** — the spike has no refundee-frozen case at all. It follows from the token's transfer rule rather than from an execution.
  - **the HTLC contract frozen** — **both revert** (`test_frozen_CONTRACT_bricks_both_paths`, which asserts the revert on `claim`, warps past the timeout, and asserts the revert on `refund`). Not recoverable, by anyone, ever. No timeout rescues it. **This is the row that was actually executed, and it is the row that matters.**
- **What that does to the guarantee:** the native corridor's promise — *if either party walks away, both can refund after their timelock* — holds unconditionally, because nobody can stop an ETH transfer. Here the same sentence carries **"…unless the issuer intervenes."** Atomicity becomes **conditional on a third party not acting**: strictly weaker than the native ETH, BTC or RXD legs, and **no amount of Solidity fixes it**, because it is the token contract and not the HTLC that decides whether a transfer moves.
- **Controls — what BOUNDS it (none of them is a fix):**
  - **Per-swap contracts, as a security property rather than a style choice.** `Erc20HtlcLeg.fund` deploys a fresh CREATE address per swap (`src/pyrxd/eth_wallet/erc20_leg.py`), so a contract freeze loses exactly one swap. A shared multi-swap HTLC would be a **single freeze-point for every swap at once**; that is the measured argument for this topology, and why the sibling repo's shared `HashedTimelock` design was not adopted.
  - **A pre-reveal gate, inside the dangerous operation.** `assert_not_frozen_before_reveal` (`src/pyrxd/eth_wallet/erc20.py`) runs inside `Erc20HtlcLeg.claim` immediately before it delegates the broadcast — not beside it, and not as something a caller has to remember. It was defined and reachable from **no production module at all** for a full review cycle; `tests/test_erc20_freeze_gate.py::TestTheGateIsActuallyINVOKEDNotMerelyAvailable` now asserts invocation rather than availability.
  - **Read at the TIP, deliberately, unlike every other verification in this stack.** `is_blacklisted` reads at `block_identifier="latest"` rather than the finalized checkpoint `verify_funded` uses, because a checkpoint read lags fresh freezes — and a fresh freeze is the only thing this check exists for.
  - **The HTLC address cannot be omitted.** It is a required, separate parameter rather than an entry in a free-form dict, because a caller passing only the two party addresses would run the gate while skipping the one case that has no recovery.
  - **Capability is PINNED, not probed.** `Erc20Token.has_blacklist` and `blacklist_fn` (`src/pyrxd/eth_wallet/tokens.py`) record whether a token can freeze and what its predicate is called: measured 2026-08-25 against the live contracts, USDC answers `isBlacklisted` and reverts `isBlackListed`, and L1 USDT does the exact reverse. A probe cannot distinguish "no such function" from "could not reach the chain", and reading either as "not frozen" is a fail-OPEN in the one gate guarding an unrecoverable outcome. `is_blacklisted` therefore **raises** when it cannot read, and the registry default is `has_blacklist=True` so a new entry fails safe.
  - **A refusal does not destroy the secret.** Both the unreadable lane and the genuinely-frozen lane raise `PreRevealAbort`: nothing was broadcast, so the preimage is still secret and the swap can retry or refund. A "frozen" verdict comes from a single unauthenticated read and blacklists are reversible, so treating it as terminal would burn the only copy of `p` on one provider's answer.
  - **A short funded window is itself a safety property here** — exposure is the time the tokens sit in the contract. That is guidance for whoever chooses the timeouts, **not an enforced bound**, and it cuts against the usual instinct to be generous with them.
  - Tests: `tests/test_erc20_freeze_gate.py`, `tests/test_erc20_freeze_fn_per_token.py`, plus the real-blacklist reachability check in `tests/test_erc20_leg_fork_integration.py`.
- **Two gates, checking different addresses, because a different transfer is at risk in each.** The reveal gate deliberately does NOT check the **refundee**: a `claim` sweeps to the claimant and never touches the refundee, so refusing there would be a guard refusing valid work — and would hand the counterparty a free unilateral veto over a leg the claimant has already earned. The refundee's freeze matters *before funding*, and `assert_not_frozen_before_funding` (`src/pyrxd/eth_wallet/erc20.py`) is the gate for it: `refund()` **pays** the refundee, so funding a leg whose refundee is already frozen buys a position with no exit — if the counterparty simply never claims, the tokens stay in the contract for good. It runs from `Erc20HtlcLeg.fund` twice: once before the deploy spends gas, and once **inside** the branch that moves tokens, where the contract address is known and its freeze is the unrecoverable one.
  - **Scoped to "something is about to be sent", deliberately.** A resume whose push already landed has nothing left to risk and is calling `fund` only to recover its locator; refusing it would leave a taker whose tokens *are* in the contract unable to record the fund that completed, while un-funding nothing. That case is pinned by a test, because the first version of this gate got it wrong.
  - It is the same **narrow** promise as the reveal gate: one read at the tip. A freeze landing between the check and the transfer is unmitigated. What it removes is the case where the freeze was already there and nothing looked. Tests: `tests/test_erc20_prefund_freeze_gate.py`.
- **A DIFFERENT residual on the tokens that cannot freeze:** `has_blacklist=False` is pinned for the OP-stack bridged USDT on Optimism and Base, and it is **positively established rather than inferred from a failed probe** — verified 2026-08-25 as recorded in `src/pyrxd/eth_wallet/tokens.py`: `l1Token()` returns Tether's real L1 contract, the EIP-1967 implementation and admin slots are empty (not a proxy), an opcode-aware disassembly finds no `DELEGATECALL`/`CALLCODE`/`SELFDESTRUCT`/`CREATE*`, and the only privileged entry points are bridge-gated mint/burn. That is **not** the same as "no counterparty risk": **burn is bridge-callable against an arbitrary holder, and the L2 bridge is itself an upgradeable system contract.** That is bridge-governance risk rather than issuer discretion — a different trigger, a different operator — and **the pre-reveal gate does not cover it at all**, because the gate only knows about freezes. Linea USDT stays unpinned for the inverse reason: it *is* a proxy with a populated admin slot, and "cannot freeze today" is not the property worth pinning.
- **Residual risk:** REAL, **ACCEPTED and DISCLOSED** — bounded, never solved. Check-then-reveal is **itself a race**: the gate narrows the window and cannot close it, and a freeze landing after the reveal is unmitigated by construction. It is a seatbelt, not a fix. The freeze read is **single-source** — it defends a *failing* provider, not a *lying* one, so a hostile RPC answering "not frozen" for a frozen address is uncaught; multi-source quorum is the fix if this corridor ever carries real value, and it is the same single-provider residual the finality path already carries. The refusal side of the gate has been exercised only against the token's real bytecode on a **fork**, where the blacklister can be impersonated; **no live-network freeze has been observed by this stack**, and the swap stack remains unaudited. Any user-facing description of this corridor must carry the issuer-trust caveat next to the existing "swaps are unaudited" line: **never present an RXD↔USDC swap as trustless, or as equivalent to the native one.**
...
| Sole-authority audit gate — **advisory since 0.9.0; does NOT block** | TA6 | `src/pyrxd/spv/proof.py:require_spv_sole_authority_cleared` |
...
| Taker-side asset-funding gate: covenant SPK re-derived from the taker's own terms + exact on-chain value + depth, before any counter leg is locked | S24 | `src/pyrxd/gravity/swap_coordinator.py:taker_verify_asset_funding`, `gravity/radiant_leg.py:verify_maker_asset_funded` |
| Asset-funding verification re-run at lock time inside `taker_funds_btc` (verify→lock TOCTOU) + non-skippable step of `pre_btc_lock_check` | S24 | `src/pyrxd/gravity/swap_coordinator.py:pre_btc_lock_check`, `taker_funds_btc` |
| Fee-pool cap charges SPEND, not DISPENSE: a refused (never-broadcast) build returns its cap charge | S21 | `src/pyrxd/gravity/capped_fee_source.py:release_unspent`, `gravity/radiant_leg.py:_unspent_on_failure` |
| Cold recovery refuses an unbounded fee overpay and a 0-conf covenant parent | S21 | `src/pyrxd/cli/swap_recovery.py:select_fee_utxo`, `_assert_covenant_confirmed` |
| `outspend`/tx reads bind the echoed txid, reject non-finite numbers, and treat only `spent is False` as unspent | TA6, S23, S24 | `src/pyrxd/network/bitcoin.py:_assert_tx_identity`, `_finite_int`, `read_confirmed_unspent_output` |
| An "already known" broadcast rejection is honored only when a read-back produces the same bytes | TA6 | `src/pyrxd/network/failover.py:_holds_tx` |
...
| Per-swap CREATE HTLC for a token leg (never a shared multi-swap contract), so an issuer freeze on the contract loses one swap | S25 | `src/pyrxd/eth_wallet/erc20_leg.py:fund/_deploy` |
...
7. **Certificate pinning for ElectrumX TLS is opt-in, and off by default.** A CA compromise
   enables TA4 against the default configuration, which relies on the system trust store.
   Since 0.14.0 an operator may pin SPKI hashes (`src/pyrxd/network/tls_pin.py`), and failover
   holds every endpoint to the same pin set; a pin mismatch fails closed and is never routed
   around. Unpinned remains the default, so the residual stands for anyone who has not
   configured pins.
8. **The SPV primitive is not a self-sufficient sole authority.** It enforces the committed nBits pin and per-header PoW but does **not** do most-cumulative-work selection or independent network-difficulty oracling (audit F-01). It is safe only behind an on-chain covenant pinning nBits. `require_spv_sole_authority_cleared` is **advisory and does not block**: since 0.9.0 it returns unconditionally, matching the project's posture that audit gates warn rather than hard-block (Radiant Core itself ships unaudited without blocking mainnet use). A covenant-less use (bridge-in/oracle/gate) is therefore restrained by **documentation, not by code** — the caller is responsible for not making Python the sole difficulty authority over real value. See [`docs/how-to/spv-verification-pitfalls.md`](how-to/spv-verification-pitfalls.md).
...
13. **dMint V1 deploy + PoW mint: now regtest-consensus-validated.** The earlier "documented but not implemented" wording was stale — the builders + reference miner shipped; the real gap was node validation. `tests/test_dmint_v1_regtest_e2e.py` proves on a real `radiant-core` node that a pyrxd-built V1 deploy (commit→reveal genesis) and a PoW-mined mint are accepted, a wrong nonce is rejected, and the contract recreates at height+1. Surfaced a consensus requirement the golden vectors missed: V1 contracts MUST be 1-photon singletons (covenant enforces `OP_OUTPUTVALUE==1`); `build_dmint_mint_tx` now rejects non-1 carriers early. **dMint V2 is now consensus-validated too (#219):** the canonical-Photonic redesign is byte-matched to upstream and accepted on `radiant-core` v3.1.1 regtest (FIXED + LWMA, with on-chain difficulty advancement) AND Radiant mainnet 3.1.2: the first V2 FIXED deploy + PoW mint confirmed on mainnet (deploy `95335028…bb16fb09`, mint `1239f64a…e0cd6c67`), plus an LWMA mint that lowered the recreated target on-chain (MAX → ~MAX/8) exactly matching the off-chain DAA (deploy `dea3beb9…`, mint `e7b52f16…`) — so adaptive difficulty is proven on mainnet, not just regtest. The per-call `V2UnvalidatedWarning` is no longer emitted, and as of 0.9.0 V2 is the **default** deploy format: `allow_v2_deploy` defaults to `True` and the historical `allow_v2_deploy=False` opt-out is itself deprecated (soft-warns). All five DAA modes are now ported and byte-matched to canonical Photonic, and the `glyph deploy-dmint --v2` / `claim-dmint` CLI verbs expose V2 deploy + PoW mint. **EPOCH int64-overflow: found, fixed upstream, re-enabled.** Differential testing of the ported modes surfaced an int64-overflow in the *canonical Photonic* EPOCH (and LWMA) bytecode — the on-chain retarget computed `target × clampedDelta` (multiply-first, output capped at `MAX_TARGET` not `2^48`), which exceeds int64 (`CScriptNum`) for ordinary parameters and aborts the mint with `INVALID_NUMBER_RANGE_64_BIT` (`OP_MUL → safeMul`), permanently bricking the contract (a liveness bug, not a theft vector — confirmed against `radiant-core` `interpreter.cpp`). EPOCH was temporarily refused at deploy while the canonical bytecode was broken; the fix is now **merged upstream** ([`Radiant-Core/Photonic-Wallet#2`](https://github.com/Radiant-Core/Photonic-Wallet/pull/2) — EPOCH clamps the target to `EPOCH_MAX_SAFE_TARGET` (2^48) on *both* sides of the multiply and divides first, so the intermediate stays ≤ 2^52 for any reachable state; LWMA floors `timeDelta` at 0 via `OP_0 OP_MAX`). pyrxd **byte-matches the merged canonical** (EPOCH+LWMA golden vectors regenerated; the off-chain miner replicas `compute_next_target_epoch`/`_linear` updated to match), and `DmintV2DeployParams` / the CLI now accept `DaaMode.EPOCH` again (difficulty ≥ 32768 for the 2^48 cap). The off-chain `current_time` validation is retained: before grinding, the mint builder refuses a locktime above `0x7FFFFFFF`, a locktime below 2^23 for ASERT, LWMA and EPOCH, a backwards locktime on the 2026-06-16 pre-floor LWMA, any mint whose recreated target would be 1, and any input its retarget mirror cannot evaluate in int64. **2026-09-16 resync to ASERT-v2 / LWMA-v2.** Photonic then replaced the ASERT retarget with a fractional fixed-point formula (`ed53cd41`, 2026-06-19) and the unity-gain LWMA with the same damped formula (`c90e6506`, 2026-06-20); pyrxd did not follow for three months, so its ASERT/LWMA deploys in that window bake the retired formulas. New deploys now byte-match `script.ts` @ `becf41a7`; the retired builders are kept frozen, `detect_contract_daa_bytecode` reads which generation a contract bakes, and `build_dmint_mint_tx` recomputes the target with the matching mirror — verified against the chain for the mainnet `dea3beb9…` deploy, whose on-chain mint `e7b52f16…` the builder recreates byte-for-byte (`tests/test_dmint_daa_v2_resync.py`). A contract whose bytecode matches no known generation is refused before the PoW grind, not mined under a guess. Residual: this newer surface is unaudited — verify it yourself before moving real value.
...
- **2026-08-25** — added **TA10** (token issuer / bridge governance — the first actor in this document that is inside the *asset* rather than outside the protocol) and **S25** (issuer freeze strands an ERC-20 counter leg) for the USDC/USDT counter leg. Records the measured fact that freezing the **HTLC contract address** reverts `claim` *and* `refund` with no timeout that recovers them, so atomicity on this corridor is conditional on the issuer not intervening; the pre-reveal gate is a seatbelt, not a fix, and check-then-reveal is itself a race. Also records the distinct **bridge-governance** residual on the OP-stack bridged USDT whose `has_blacklist=False` is positively established — the pre-reveal gate does not cover a bridge-callable burn. Four new rows in "Controls in place".
- **2026-08-10** — added **S24** (maker never locks the asset and sweeps the taker's counter leg — hazard HZ-1, the mirror of S23 and the cheaper attack: loss is the FULL counter leg, not a shortfall). The taker's asset-funding check previously existed only inside the operator scripts, so any caller driving `SwapCoordinator` directly locked its counter leg without reading the Radiant chain at all; the library now re-derives the covenant scriptPubKey from the taker's own terms, binds the exact on-chain value and a policy-pinned depth, and re-runs the check at lock time. Also **corrected the S21 claim** that the claim executor "does not retry into a fee-pool drain" — it does retry, and that is only safe now that a refused build returns its cap charge (`CappedFeeWalletSource.release_unspent`); the measured drain is recorded inline. Seven new rows in "Controls in place".
