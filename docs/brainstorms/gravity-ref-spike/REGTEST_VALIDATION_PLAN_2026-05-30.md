...
> **Why V-1/V-2/V-3 are HIGH and V-4 is MEDIUM:** 5/6/7 significant bytes are **reachable** on the real anchored chain — 42.9 BTC (V-1) and 21M-BTC MAX_MONEY (V-2/V-3) are genuine high-value swaps/offers. An 8-byte value (V-4) exceeds MAX_MONEY ~4000× and cannot occur. **Divergence hypothesis for the whole group: suspected NO divergence** — the live mainnet dMint contract decodes an 8-byte target via `OP_8 OP_SPLIT` + `OP_GEQ` in production (`docs/dmint-research-mainnet.md:118-141`), proving Radiant raised the script-integer limit to 8 bytes/64-bit. If that assumption is **wrong** (legacy `nMaxNumSize=4` retained), V-1/V-2/V-3 are all Direction-A taker fund-loss.
...
**Relevant file paths:** `/home/eric/apps/pyrxd/tests/test_spv_covenant_differential_deployed.py` (skips to un-skip: lines 576-583, 716-724; xfail 651-665; "NOT modelled" 143-148), `/home/eric/apps/pyrxd/tests/test_htlc_regtest_e2e.py` (`_RegtestNode` 66-147, fixture 150-164, `_pay_to_spk` 194-248), `/home/eric/apps/pyrxd/src/pyrxd/gravity/transactions.py` (`build_finalize_tx` 555-695; sentinel pad 660-661; floor 613; over-depth 658-659), `/home/eric/apps/pyrxd/src/pyrxd/gravity/covenant.py` (`build_gravity_offer` 328; `_encode_int_push` 87; `_Nbits` guard ~486; `validate_claim_deadline` 276), `/home/eric/apps/pyrxd/src/pyrxd/gravity/codehash.py` (P2SH helpers 30-58), `/home/eric/apps/pyrxd/src/pyrxd/spv/proof.py` (`SpvProofBuilder.build` 315), `/home/eric/apps/pyrxd/src/pyrxd/security/types.py` (Nbits exp cap 203-204), `/home/eric/apps/pyrxd/src/pyrxd/gravity/artifacts/maker_covenant_flat_12x20_sentinel_all.artifact.json` (covenant ABI/ASM ground truth). **New file to create:** `/home/eric/apps/pyrxd/tests/test_spv_covenant_differential_regtest.py`.