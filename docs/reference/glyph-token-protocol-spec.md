...
`src/pyrxd/glyph/types.py:99-137`. Both halves are in human-readable order, so the
...
(`src/pyrxd/glyph/payload.py:68`) — RFC 8949 deterministic form: map keys sorted
...
| `name` | text | no | 200 | Display name. 200 is the cap RXinDexer's glyph index applies (`electrumx/server/glyph_index.py:1465` at `ca8a6a4e`, which truncates; pyrxd's decoder drops). It was 64 until #728, which dropped the qualified name of every WAVE claim with a 61-63 character label: the label plus `.rxd` is up to 67 characters, and Photonic writes it |
...
- **`decimals`** is display metadata only. On chain, one photon is one FT unit;
  ...
  CBOR floats truncate silently (`src/pyrxd/glyph/payload.py:80-90`).
...
(`src/pyrxd/glyph/builder.py` `GlyphBuilder.prepare_commit`).
...
- The reveal's recipient MAY differ from the key that signs the reveal. pyrxd
  ...
  supported flow (`src/pyrxd/glyph/builder.py` `RevealParams`).
...
(`src/pyrxd/glyph/builder.py` `GlyphBuilder.prepare_mutable_reveal`). Both carry the same ref.
...
(`src/pyrxd/glyph/builder.py` `GlyphBuilder.prepare_container_reveal`). There is no container script shape.
...
State script (`src/pyrxd/glyph/dmint/builders.py` `build_dmint_v1_state_script`):
...
`0xef` = K12 (`src/pyrxd/glyph/dmint/builders.py` `build_dmint_v1_code_script`).
...
`src/pyrxd/glyph/dmint/builders.py` `build_dmint_state_script`. `height` and `target` use **minimal**
...
(`src/pyrxd/glyph/builder.py` `_assert_declared_dmint_matches`) — a deliberate addition, not shared with
...
**WAVE registration fee.** A reveal that registers a WAVE name pays the
protocol's registration fee: an output of the name's price to the P2PKH of the
WAVE treasury, `1GrwkQNJfjbEJjH25heszNZLpbZou8nfXG`. The price is set by the
length of the bare label: 100 RXD for 3 characters, 50 for 4, 10 for 5, and 5
for 6 to 63. The WAVE protocol says registration is "a one-time, length-based
fee" and that renewal pays "the name's registration price to the protocol
treasury" (Radiant-Core/WAVE-Protocol `ANNOUNCEMENT.md:51-52` at `c05b8e7a`);
the amounts and the address are Photonic's (`calculateNameCost`,
`packages/lib/src/wave.ts:55-66`;
...
envelope is encoded with (`src/pyrxd/glyph/payload.py:68`). Signing and
...
|---|---|---|---|
| Envelope encoding | Producer-order CBOR (the mainnet fixture is non-canonical — §14) | RFC 8949 canonical | Determinism across source refactors and re-encoders (`src/pyrxd/glyph/payload.py:29-69`). Consumers on both sides must accept either (§4.2). |
...
| V1 dMint `maxHeight` at deploy | `dMintScript` builds any `maxHeight`; mainnet has V1 contracts above 2^31 (`$BRO`: 696,969,000,000) | Refused above 2^31. Such contracts are still read and minted up to height 2^31 − 1, and the mint from there is refused | A V1 contract's height is a 4-byte field that every mint but the last rewrites as `NUM2BIN(height + 1, 4)`, which cannot encode 2^31, so above that `maxHeight` the contract stops at height 2^31 − 1 with mints left (`src/pyrxd/glyph/dmint/types.py` `MAX_V1_MAX_HEIGHT`, `check_dmint_v1_bounds`). |
...
(`src/pyrxd/glyph/builder.py` `_validate_premine`, citing `src/policy/policy.cpp:19-25` at
`v3.1.2`). The 1-photon value on dMint contract outputs is pinned by the
**covenant** (`OP_OUTPUTVALUE OP_1 OP_NUMEQUALVERIFY`,
`src/pyrxd/glyph/dmint/builders.py` `_build_part_c`), not by the chain. The 546-photon floors
...
use it (`src/pyrxd/glyph/scanner.py:317-353`), which is lenient rather than
normative.
