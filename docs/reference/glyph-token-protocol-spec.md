...
`src/pyrxd/glyph/_inspect_core.py:380-420`).
...
`src/pyrxd/glyph/_inspect_core.py:1043-1198` is the dispatch order pyrxd uses — NFT, FT,
container-legacy, MUT, commit-NFT, commit-FT, in that order — and
`src/pyrxd/glyph/_inspect_core.py:1374-1436` is the envelope-side classification, which returns
