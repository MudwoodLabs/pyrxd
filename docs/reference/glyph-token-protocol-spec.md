...
| WAVE name location | Bare label in `attrs.name`, domain in `attrs.domain`, qualified name at top-level `name`, plus `v: 2` and `type: "wave_name"` (`createWaveNameMetadata`, `packages/lib/src/wave.ts`) | The same since #728, byte for byte against a mainnet claim (`tests/test_wave_claim_registers_with_the_indexer.py`). **Through 0.24.0 pyrxd put the qualified name in `attrs.name` and left top-level `name` empty.** RXinDexer's `validate_wave_name` refuses the `.`, so by its source those claims are not indexed and their names do not resolve (read from source; no pyrxd-built claim has been checked against a live indexer). Readers still accept that shape and report it verbatim; the envelope writers refuse to build it (`src/pyrxd/glyph/payload.py` `refuse_qualified_wave_label`). A top-level-only `name` (older pyrxd) is still accepted by `GlyphBuilder.prepare_wave_reveal` and is not indexed either | RXinDexer registers from `attrs.name` and rebuilds from the top-level name on a backfill; pyrxd's shape was checked against neither until #728. Photonic always writes `attrs.expires` (now + 2 years); pyrxd writes it only when given, since RXinDexer takes the term from the block time. |
