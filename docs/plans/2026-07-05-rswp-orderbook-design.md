...
| D8 | Asset scope: RXD + Glyph FT at first; **NFT singletons added later** (PR #285, shipped) | Originally NFT was decode/display-only to match `pyrxd.swap` v1 scope; the NFT singleton path (exactly-one-in/one-out discipline) landed as its own PR and is now fully post/take/cancel-able through the book. Audit follow-up: this row and the bridge docstring were corrected from the stale "NFT display only" wording. |
