...
| **Key material** | `src/pyrxd/hd/` (`wallet.py`, `bip32.py`, `bip39.py`, **(0.14)** `descriptor.py`), `src/pyrxd/security/` (`secrets.py`, `types.py`, `errors.py`), `src/pyrxd/keys.py`, `src/pyrxd/base58.py` | AES-256-GCM + scrypt wallet; `SecretBytes`; the agent's transient-xprv re-derivation; the persisted-coin-type pin (`hd/wallet.py:659-689`); NFKD seed normalization + the `normalize=False` recovery escape (0.12.0); descriptor export (an xpub is a whole-history disclosure — the module says so); the static-message base58/WIF error path (0.14.0 — this is where a key-echo bug lived, §7). |
...
- **`ASSUME-SINGLE-SOURCE` (gap #6).** RXD-side reads trust a single source by design:
  ...
  *registry* with failover and per-endpoint genesis-hash chain verification
  ...
  (`MultiSourceRxdChainSource`, `watch/adapters.py:142`, ≥2 sources, fail-closed below quorum).
...
- **`ASSUME-AS-IS-POSTURE`** (was `ASSUME-PRE-AUDIT-GATE`). As of 0.9.0 the library-wide
  ...
  no-ops** — verified at this commit: `spv/proof.py:39-55` documents "no longer blocks" and
...
2. **The ref-opcode walker diverged from Radiant consensus on `0xd4`–`0xd7`** (opcodes that
   ...
   Radiant's `GetScriptOp` (`tests/test_glyph.py:1624 TestRefWalkerConsensusDifferential`).
...
|---|---|---|---|---|
| `SPV-SOLE-AUTHORITY` | high | mitigated (covenant pin) | No most-cumulative-work selection / difficulty oracle; safe only behind a covenant nBits pin. NOTE: the `require_spv_sole_authority_cleared` "gate" is an advisory **no-op** as of 0.9.0 (`spv/proof.py:39-55`) — the covenant pin is the defense, not the gate; TM gap #8's "fails closed" wording is stale | `spv/chain.py`, `proof.py` · F-01 / TM gap #8 |
...
| ID | Sev | Status | Residual | Where / legacy |
|---|---|---|---|---|
| `GLYPH-OWNERPKH` | high | mitigated (CLI) | The CLI mint paths now derive `owner_pkh` from the funding wallet (not from the metadata file) and the pre-broadcast summary prints it — `owner_pkh: … (this wallet)` (`cli/glyph_cmds.py` `_mint_nft_inner`, `_deploy_ft_inner` and `_deploy_dmint_inner`). TM S7 / gap #9's "summary does not surface owner_pkh" wording predates this. The hostile-metadata residual has **moved**, not vanished: see `GLYPH-METADATA-DROPPED-FIELDS` | `cli/glyph_cmds.py` · TM S7 / gap #9 |
