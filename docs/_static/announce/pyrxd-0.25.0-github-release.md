...
- `pyrxd glyph mint-nft` asks an indexer whether the name has a confirmed registration before the commit, and twice more before the reveal is broadcast, and refuses one that is registered. A rival claim that is not yet mined is not seen.
- It refuses to commit again for a name this wallet already has an unrevealed commit for; `--ignore-pending-mint` overrides.
...
A mint interrupted after its commit no longer loses track of it. The command saves a record before broadcasting, and every exit after the commit prints the exact `pyrxd glyph resume-mint <txid>` command that finishes it. For a WAVE name that command states the fee choice, because `resume-mint` takes it from its command line and only cross-checks it against the record. A record is archived, never deleted, once the server reports the reveal confirmed, and success says the confirmation is the server's report.
...
- whether a WAVE name pointed at the signing key at the mark's block (`--wave-name`; two servers on different hosts must report the same block heights, and the name's glyph must name that label in its own mint);
...
- **`glyph inspect`**:
  - Shows whether a reveal's metadata is bound (`payload_binding`): its commit committed to exactly that payload, and the transaction creates the commit's token. The headline is now a payload the transaction mints, when one does.
  - Answers HashMark §7.6 form 2 with `--wave-name`, under the same two-server rule as `pyrxd verify`.
...
- **`WaveResolver.check_available`** reported every name as available. It now reports registered names correctly. The indexer answers from mined blocks, so a claim not yet mined is not seen.
...
- **WAVE labels:** `build_wave_metadata` and every writer refuse labels outside the rule above. That includes non-ASCII names, which must be written as punycode, 1–2 character names and uppercase labels. `attrs.name` now holds the bare label.
- **WAVE fee in the SDK:**
...
- **`mint-nft` and `timelock-mint`** now wait for the reveal to confirm, and exit 2 if it has not confirmed by the timeout. Success now says the server reports the reveal confirmed, rather than "NFT minted!", and names the command that reveals the commit again through another server.
- **`judge_name_at_mark`** requires `name=` and `height_reports=` (one `HeightReport` per endpoint) in place of `step_heights=`. A `ChainStep` built without the new `envelope_cbor` always degrades to form 1.
- **`GlyphMinter`** archives a finished mint's record with the new `PendingStore.archive()` instead of calling `delete()`. A custom store keeps the record unless it overrides `archive()`.
- **`verify_attestation`** raises `ValidationError` for a genesis that is not 64 lowercase hex, where 0.24.0 returned `INVALID_SIGNATURE`.
- **Commit scripts:** `is_commit_script` and the `extract_*_from_commit_script` helpers accept only the `OP_1` and `OP_2` ref types.
