...
def _seconds(tl: dict, *, i_btc: float, i_rxd: float, side: str) -> float:
    """Convert a published Timelock to seconds WITHOUT touching pyrxd.

    Deliberately re-implemented from the spec's own words — "each leg multiplied by ITS OWN
    chain's block interval" — rather than calling ``normalize_to``. A vector suite generated by
    pyrxd and validated by pyrxd only ever proves pyrxd agrees with itself; that circularity is
    how schema /1 published the inverted ordering and /2 published a unit conflation, each with a
    fully green suite behind it.
    """
    value, unit = int(tl["value"]), tl["unit"]
    if unit == "seconds":
        return float(value)
    if unit != "blocks":
        raise AssertionError(f"unknown unit {unit!r}")
    return value * (i_rxd if side == "rxd" else i_btc)


@pytest.mark.parametrize("vec", _MARGINS, ids=lambda v: v["id"])
def test_published_verdict_re_derives_from_the_INEQUALITY_not_from_our_gate(vec: dict):
    """Triangulate: published verdict == independent arithmetic == what the gate does.

    The middle term is the point. ``test_margin_verdicts`` already runs each vector through
    ``assert_timelock_margin`` — but that is the function these vectors were generated from, so
    it cannot detect the two failures this file has actually shipped: a verdict set that is
    self-consistent and wrong.

    Here the verdict is recomputed from the wall-clock relation stated in the spec

        t_rxd * i_rxd  >=  t_btc * i_btc  +  margin * i_btc

    using nothing but the numbers in the JSON. If the gate is ever changed so that it and the
    published rule disagree, this fails even though the suite it generated still passes.
    """
    i_btc, i_rxd = vec["block_interval_s"], vec["rxd_block_interval_s"]
    rxd_s = _seconds(vec["t_rxd"], i_btc=i_btc, i_rxd=i_rxd, side="rxd")
    btc_s = _seconds(vec["t_btc"], i_btc=i_btc, i_rxd=i_rxd, side="btc")
    margin_s = _seconds(vec["margin"], i_btc=i_btc, i_rxd=i_rxd, side="btc")

    independent = "accept" if rxd_s >= btc_s + margin_s else "reject"
    assert independent == vec["verdict"], (
        f"{vec['id']}: the published verdict does not follow from the published numbers.\n"
        f"  t_rxd {rxd_s:.0f}s vs t_btc {btc_s:.0f}s + margin {margin_s:.0f}s "
        f"= {btc_s + margin_s:.0f}s\n"
        f"  independent arithmetic says {independent}; the file says {vec['verdict']}"
    )

    # ...and the shipped gate must reach the same conclusion as the arithmetic.
    policy = MarginPolicy(
        margin=_timelock(vec["margin"]),
    ...
    try:
        ...
        gate = "reject"
    assert gate == independent, (
        f"{vec['id']}: assert_timelock_margin says {gate}, the spec's own inequality says "
        f"{independent}. The gate and the published rule have diverged."
    )
