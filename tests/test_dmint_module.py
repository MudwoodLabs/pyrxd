...
class TestDmintDeployParamsValidation:
    ...
    def test_max_height_zero_raises(self):
        with pytest.raises(ValidationError, match="max_height"):
            DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=0, reward=10, difficulty=5)

    def test_reward_zero_raises(self):
        with pytest.raises(ValidationError, match="reward"):
            DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=100, reward=0, difficulty=5)

    def test_difficulty_zero_raises(self):
        with pytest.raises(ValidationError, match="difficulty"):
            DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=100, reward=10, difficulty=0)

    def test_target_time_zero_raises(self):
        with pytest.raises(ValidationError, match="target_time"):
            DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=100, reward=10, difficulty=5, target_time=0)

    def test_blake3_uses_256bit_target(self):
        p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=100, reward=10, difficulty=100, algo=DmintAlgo.BLAKE3)
...
class TestBuildDmintStateScript:
    ...
    def test_height_encoding(self):
        p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=1000, reward=50, difficulty=10, height=42)
...
class TestBuildDmintContractScript:
    ...
    def test_asert_includes_txlocktime(self):
        p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                              max_height=100, reward=10, difficulty=5,
                              daa_mode=DaaMode.ASERT, target_time=60, half_life=3600)
...
class TestBuildPowPreimage:
    ...
    def test_second_half(self):
        pre = build_pow_preimage(self._TXID_LE, self._CREF, self._IN_SCR, self._OUT_SCR)
        sha256d = lambda d: hashlib.sha256(hashlib.sha256(d).digest()).digest()
...
def test_all_9_variants_produce_valid_contract(algo, daa_mode, half_life, target_time):
    p = DmintDeployParams(
        ...
        algo=algo, daa_mode=daa_mode, target_time=target_time,
        half_life=half_life, last_time=_BASE_LAST_TIME,
    )
...
def test_max_sha256d_target_at_difficulty_1():
    p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                          max_height=100, reward=10, difficulty=1)
    assert p.initial_target == MAX_SHA256D_TARGET


def test_height_in_state():
    p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                          max_height=100, reward=10, difficulty=10, height=999)
...
def test_last_time_in_state():
    ts = 1_777_103_647
    p = DmintDeployParams(contract_ref=_CONTRACT_REF, token_ref=_TOKEN_REF,
                          max_height=100, reward=10, difficulty=10, last_time=ts)
    state = build_dmint_state_script(p)
...
from pyrxd.glyph.payload import encode_payload, decode_payload
...
def test_glyph_metadata_v2_version_field():
    ...
    assert d["v"] == 2
    assert d["p"] == [1, 4]
    assert list(d.keys())[0] == "v"  # v comes first
...
def test_dmint_cbor_payload_validation_errors():
    with pytest.raises(ValidationError):
        DmintCborPayload(
            algo=DmintAlgo.SHA256D, num_contracts=0, max_height=100,
            reward=10, premine=0, diff=1,
        )
    with pytest.raises(ValidationError):
        DmintCborPayload(
            algo=DmintAlgo.SHA256D, num_contracts=1, max_height=0,
            reward=10, premine=0, diff=1,
        )
    with pytest.raises(ValidationError):
        DmintCborPayload(
            algo=DmintAlgo.SHA256D, num_contracts=1, max_height=100,
            reward=10, premine=0, diff=0,
        )
