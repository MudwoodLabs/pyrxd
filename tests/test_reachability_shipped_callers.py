...
_KNOWN_UNREACHED: dict[str, str] = {
    ...
    "src/pyrxd/glyph/timelock.py::add_timelock_to_metadata": "#556: the WRITE half (mint + reveal). The READ half is wired as of 2026-09-02 — the decoder now keeps crypto.timelock and is_unlocked/get_unlock_remaining/verify_cek_reveal are exported. This half publishes a CEK ON-CHAIN and has never had a production caller or a real-value run, so it stays unwired until it does",
    "src/pyrxd/glyph/timelock_reveal_tx.py::create_reveal_proof": "#556: the WRITE half (mint + reveal). The READ half is wired as of 2026-09-02 — the decoder now keeps crypto.timelock and is_unlocked/get_unlock_remaining/verify_cek_reveal are exported. This half publishes a CEK ON-CHAIN and has never had a production caller or a real-value run, so it stays unwired until it does",
    "src/pyrxd/glyph/timelock_reveal_tx.py::parse_reveal_proof_script": "#556: the WRITE half (mint + reveal). The READ half is wired as of 2026-09-02 — the decoder now keeps crypto.timelock and is_unlocked/get_unlock_remaining/verify_cek_reveal are exported. This half publishes a CEK ON-CHAIN and has never had a production caller or a real-value run, so it stays unwired until it does",
    "src/pyrxd/glyph/timelock_reveal_tx.py::validate_reveal_proof": "#556: the WRITE half (mint + reveal). The READ half is wired as of 2026-09-02 — the decoder now keeps crypto.timelock and is_unlocked/get_unlock_remaining/verify_cek_reveal are exported. This half publishes a CEK ON-CHAIN and has never had a production caller or a real-value run, so it stays unwired until it does",
