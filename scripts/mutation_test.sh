...
# substitution fail, but the command substitution still expands to the EMPTY string, and
# `cd ""` succeeds (returns 0, no-op) rather than erroring — so the guard never fired.
...
group_tests() {
  ...
  case "$1" in
    ...
    wire)        echo "tests/test_compactsize.py tests/test_merkle_path.py tests/test_script_consensus.py tests/test_rswp_wire.py tests/test_watch_claim_executor.py tests/cli/test_glyph_cmds.py tests/test_coverage_gaps4.py tests/test_coverage_gaps5.py tests/test_ref_walker_differential.py tests/cli/test_swap_recovery_cmds.py tests/test_radiant_leg.py tests/test_hashmark_attestation.py tests/test_hashmark_mainnet_vectors.py tests/test_hashmark_decoder.py tests/test_hashmark_encoder.py tests/test_hashmark_mark_cli.py tests/test_message_data_carrier.py tests/test_op_return_push_encoding.py tests/web/test_inspect_imports_pyodide_clean.py tests/test_rswp_orders.py tests/test_rswp_covenant_ft_demand.py" ;;
    ...
    # never executed them and rxindexer.py scored 12% killed with every wave_reverse_lookup branch
    # "untested". tests/network/test_rxindexer.py holds the boundary cases those two still miss.
