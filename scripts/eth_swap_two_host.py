...
# "mnemonic" cover raw entropy and BIP32/BIP39 material. KEEP IN SYNC with btc_swap_two_host.py and with
...
def _assert_public_only(doc: dict, *, what: str) -> None:
    ...
    secret into the out-of-band channel. Scans keys (private-material markers) recursively, and str
    VALUES for a WIF shape (a secret smuggled under a benign key name), case-insensitively.
    """
...
def _terms_from_public(
    *,
    ...
    t_rxd_blocks: int,
    margin_blocks: int,
    # The two chains' block intervals, needed because t_btc derives in WALL CLOCK (#567).
    # Defaulted so the self-check and any other in-module caller need not thread them.
    rxd_block_interval_s: float = 300.0,
    btc_block_interval_s: float = 600.0,
...
def _rederive_covenant(args: argparse.Namespace, *, terms: NegotiatedTerms, taker_pkh: bytes, maker_pkh: bytes):
...
#                    The taker's OWN counter leg: the ETH HTLC's refund() pays the immutable
...
#                    taker's to refund (the maker holds no counter-leg refund key), not because an
...
async def taker_phase_abort(args: argparse.Namespace) -> None:
    """TAKER recovery — get the TAKER'S OWN ETH leg back and stop (``--phase abort``).

    Drives ``SwapCoordinator.taker_refund_btc()`` (BTC_LOCKED -> ABORTED). Use it the moment the
    ETH HTLC's timeout has passed: it needs nothing from the maker, and it needs no Radiant read,
    so it still works when the Radiant node is unreachable.
...
async def maker_phase_refund(args: argparse.Namespace) -> None:
    ...
    refund — the maker holds no ETH refund path here — not because an asset-only refund is a general
    ...
    ETH claim it has already been paid, and refunding the asset as well would take BOTH legs. The
    ...
    # The maker has been paid once its ETH claim is published; the coordinator must not then refund
    ...
    try:
        ...
        if maker_has_claimed:
            print(
                "  maker_claim.json is present: the maker has already claimed the ETH, so the coordinator's "
        ...
        print(f"  -> {rec.state.value}; the covenant is refunded to the maker. The taker refunds its own ETH.")
    finally:
        await rpc.close()


async def maker_phase_abort(args: argparse.Namespace) -> None:
...
def main() -> None:
    ...
    if fn is None:
        raise SystemExit(
            f"invalid (role, phase) = ({args.role}, {args.phase}); "
