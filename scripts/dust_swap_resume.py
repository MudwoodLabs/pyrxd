...
async def resume(args) -> None:
    ...
    assert hashlib.sha256(p_secret.unsafe_raw_bytes()).digest() == h, "preimage does not hash to H!"
    ...
    t_btc = bt.Timelock(args.t_rxd_blocks + margin_blocks + 4, bt.TimeUnit.BLOCKS)
    ...
    assert cov.funded_spk.hex() == keys["rxd_covenant_spk"], "covenant SPK mismatch vs persisted!"
    ...
    print(f"  BTC HTLC locator reconstructed: {funding_txid}:0 = {on_chain_amount} sats")
    ...
    btc_leg = BitcoinTaprootLeg(
        network=btc_network, taker_keypair=taker_btc_kp,
        ...
        maker_claim_privkey=maker_btc.secret, audit_cleared=True,
    )
    rxd_leg = RadiantCovenantLeg(
        network="bc", taker_pkh=taker_pkh, maker_pkh=maker_pkh,
        chain_io=RadiantChainIO(rxd_client), fee_source=_SshTrFeeSource(rxd_client, args.rxd_fee_photons),
        min_confirmations=1, audit_cleared=True,
    )
    record = SwapRecord(state=SwapState.NEGOTIATED, terms=terms).with_btc_lock(locator).with_state(
        SwapState.BTC_LOCKED
    )
    coord = SwapCoordinator(
        record=record, btc_leg=btc_leg, radiant_leg=rxd_leg,
    ...
    while True:
        ...
        if rec.state is SwapState.COMPLETED:
            ...
            break
        print(f"  reorg gate: WAIT (BTC claim not yet {policy.btc_claim_reorg_depth.value}-deep); retrying in {args.poll_interval_s}s...")
        await asyncio.sleep(args.poll_interval_s)
