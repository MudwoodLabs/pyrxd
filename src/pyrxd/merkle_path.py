...
class MerklePath:
    ...
    async def verify(self, txid: str, chain_tracker: "ChainTracker") -> bool:
        ...
        asks *chain_tracker* to confirm the root against the on-chain block header.

        Args:
            txid (str): The transaction ID to verify (64-char lowercase hex).
            chain_tracker (ChainTracker): ChainTracker instance used to fetch and
                verify the block header's Merkle root.

        Returns:
            bool: ``True`` if *txid* is validly included at ``self.block_height``.
        """
