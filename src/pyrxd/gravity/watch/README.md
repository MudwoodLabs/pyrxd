...
- **Second-channel escalation (the remaining sliver):** time-critical CRITICAL pages re-page on a backoff and can be quieted by the operator-ACK inbox (`FileAckInbox` → `run_loop(on_tick_start=…)` → `DedupAlerter.ack`), and the un-ACK'd CRITICAL count is surfaced on the heartbeat (`unacked_critical`). What is NOT built: an *automatic* second-channel escalation that consumes that count (today an external monitor must read the heartbeat and escalate). The page→ack→heartbeat-count loop is in place; auto-escalation on a persistent count is the open piece.
