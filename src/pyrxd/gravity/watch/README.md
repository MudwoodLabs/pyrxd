...
- **`alerts.py`** — `DedupAlerter` maps a `Decision` → severity + `Page`, deduped by `(swap_id, intent)`; dedup advances only after a successful send (transient channel failures retry). Time-critical CRITICAL pages (`PAGE_CLAIM`/`PAGE_SQUEEZED`) **re-page** on a tick-count backoff (`repage_critical_every_ticks`, default every tick) until the situation resolves, so a single missed page on a claim/squeeze race cannot silently lose funds; WARN/INFO page once per situation.
- **`heartbeat.py`** — `FileHeartbeat` (the tower's cross-process beat, now versioned by `schema_version`) + `DeadMansSwitch` (**is the tower alive?**).
- **`escalation.py`** — `EscalationMonitor` (**is the OPERATOR alive?**): reads the beat's `unacked_critical` and, when it persists past a time threshold measured in the tower's own clock, pages a *distinct* channel. Fail-closed: an absent count refuses startup, the `-1` sentinel escalates immediately, a stale beat defers to the dead-man's-switch.
...
- **v1 — alert-only, BTC.** This package's core: watch + page, no broadcast, no keys, no `p`. Authenticated `WebhookAlertChannel` (HMAC) + a cross-process dead-man's-switch (`FileHeartbeat` + `DeadMansSwitch`, via `pyrxd-watchtower-deadman`) + a cross-process second-channel escalation monitor (`EscalationMonitor`, via `pyrxd-watchtower-escalate`). Entrypoints: `pyrxd-watchtower`, `pyrxd-watchtower-deadman`, `pyrxd-watchtower-escalate` (the `scripts/watchtower_*.py` paths remain as back-compat shims).
...
- **Broaden autonomous actions** (each audit-gated; each needs the live capped-fee-key custody seam `RadiantLeg.fee_source`): RXD-covenant refund (not pre-signable), `mutual_refund` (two broadcasts), the autonomous **claim** (`taker_scrape_and_claim_asset` — scrape `p`, fire a reorg-gated Glyph claim before `t_rxd`; highest value, biggest lift), and ETH-leg autonomy (verify the real `EthHtlc.sol` `refund()` guard first).
  - **Autonomous claim — BUILT and ARMABLE (`claim_executor.py`, `ClaimExecutor`), DORMANT-by-construction.** Hot-fee-key (the RXD covenant claim is not pre-signable), value-vs-reorg capped per swap (`swap_coordinator.max_protected_value` — the exact inverse of the coordinator's value-scaled burial; ft/nft need the explicit dust opt-out, `accept_unbounded_reorg_risk`), `low_corroboration` hard-stop (no consensus backstop, unlike the refund), and it **re-asserts `assess_claim_finality` SAFE against a FRESH read + covenant-still-unspent immediately before broadcasting** (a tick-open verdict is stale on a cheap-reorg chain). `decide()` arms it via the typed `Decision.autonomous_asset_claim` (BTC SAFE branch ONLY, never ETH); a `CompositeExecutor` runs it alongside the refund executor. Per-swap covenant params come from a `<swap_id>.claim.json` sidecar (`CovenantClaimContext`, the pkhs the record lacks). Claim-tx bytes via `MempoolClaimBytesSource`. **NOT REACHABLE FROM ANY ENTRYPOINT — there is no way to arm it today.** `watchtower_run.py` builds only the `RefundExecutor`; no `--arm-claim` / `--claim-rxd-wallet` flag exists anywhere in the repo (this README previously documented both, which was wrong). Wiring it up is deliberately deferred: it is a hot-fee-key autonomous broadcaster, **still gated for real value** on an independent ≥2-source RXD quorum *deployment*, a dust proof (P3), and the external audit (P4). Treat autonomous claim as UNVERIFIED in any run report.
...
- **Second-channel escalation — DONE.** Time-critical CRITICAL pages re-page on a backoff and can be quieted by the operator-ACK inbox (`FileAckInbox` → `run_loop(on_tick_start=…)` → `DedupAlerter.ack`); the un-ACK'd CRITICAL count rides on the heartbeat (`unacked_critical`); and `escalation.py` (`EscalationMonitor`, console script `pyrxd-watchtower-escalate`) is the **automatic** consumer that used to be the open piece — a separately-supervised third process that pages a **different** channel when the count persists. Threshold is measured in the *tower's* clock (beat `ts` deltas, not monitor ticks); state (`first_unacked_ts`) is persisted 0600 beside the heartbeat so a restart loop cannot rewind it; a missing count key or the `-1` sentinel is read as **blind**, never as zero (refuse-to-start / escalate-immediately respectively); a stale beat does NOT double-page the dead-man's-switch's alarm (one edge-triggered WARN instead); and it refuses to start if its webhook equals the tower's. Heartbeat payloads now carry `schema_version` and the monitor refuses one it does not know. What is still operational, not code: actually running the escalation channel on an independent transport.
