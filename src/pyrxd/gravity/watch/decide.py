...
v1 is BTC-only; an ETH (``counter_chain == "eth"``) swap yields ``NOOP`` (deferred
to v3). v1 broadcasts nothing — every actionable Intent is a *page* to the
operator, who runs the named one-shot coordinator step.
"""
...
class Intent(Enum):
    ...
    * ``NOOP`` — not this watchtower's concern (e.g. an ETH swap, deferred to v3).
    """
...
def decide(
    *,
    ...
    terms = record.terms
    # v1 is BTC-only; ETH is deferred to v3.
