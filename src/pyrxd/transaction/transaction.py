...
class Transaction:
    ...
    def fee(self, model_or_fee=None, change_distribution="equal"):
        ...
        if change_distribution == "random":
            ...
            # Integer-division remainder is routed to the first change output,
            # not silently leaked to miners. For e.g. change=10, change_count=3:
            # outputs become [4, 3, 3] (sum=10), not [3, 3, 3] (sum=9, leak=1).
            per_output = change // change_count
            remainder = change - per_output * change_count
            assigned_remainder = False
            for out in self.outputs:
                if out.change:
                    if not assigned_remainder:
                        out.satoshis = per_output + remainder
                        assigned_remainder = True
                    else:
