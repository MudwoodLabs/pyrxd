...
def _decode_attrs(raw: object) -> dict[str, str]:
    ...
    if len(raw) > _MAX_ATTRS_COUNT:
        raise ValidationError(
            f"'attrs' map too large: {len(raw)} entries > {_MAX_ATTRS_COUNT}"
        )
...
def _decode_decimals(raw: object) -> int:
    ...
    if isinstance(raw, float):
        raise ValidationError(
            f"'decimals' must be an integer, got float {raw!r}. "
            "CBOR floats truncate silently — use an integer."
        )
...
def decode_payload(cbor_bytes: bytes) -> GlyphMetadata:
    ...
    if len(cbor_bytes) > _MAX_CBOR_PAYLOAD_BYTES:
        raise ValidationError(
            f"CBOR payload too large: {len(cbor_bytes)} > {_MAX_CBOR_PAYLOAD_BYTES} bytes"
        )
    try:
...
def _push_minimal_int(n: int) -> bytes:
    ...
    raise ValidationError(f"_push_minimal_int: value too large")
