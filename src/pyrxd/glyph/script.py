...
def build_commit_locking_script(
    ...
    *,
    ...
    return (
        b'\xaa'                   # OP_HASH256
        + b'\x20' + payload_hash  # PUSH 32 + hash
        ...
        + b'\xda' + reftype_push + b'\x9d'  # OP_REFTYPE_OUTPUT <OP_N> OP_NUMEQUALVERIFY
        + b'\x76\xa9\x14' + bytes(owner_pkh) + b'\x88\xac'  # P2PKH tail
    )
...
MUTABLE_NFT_SCRIPT_RE = re.compile(
    r'^20[0-9a-f]{64}75bdd8[0-9a-f]{72}'
    + _MUTABLE_NFT_BODY.hex()
    + r'$'
)
...
def build_mutable_nft_script(mutable_ref: GlyphRef, payload_hash: bytes) -> bytes:
    ...
    script = (
        ...
        + b'\xd8' + mutable_ref.to_bytes()  # OP_PUSHINPUTREFSINGLETON + 36-byte ref
        + _MUTABLE_NFT_BODY
    )
    if len(script) != MUTABLE_NFT_SCRIPT_SIZE:
        raise RuntimeError(
            f"Mutable NFT script size invariant violated: "
            f"expected {MUTABLE_NFT_SCRIPT_SIZE}, got {len(script)}"
        )
    return script
